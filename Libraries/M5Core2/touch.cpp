...
//	return (_state && _time - _lastChange >= ms);	HACKHACK VK
	return (_state &&  millis() - _lastChange >= ms);
}

bool TouchButton::pressedFor(uint32_t ms, uint32_t continuous_time) {
	// if (_state && _time - _lastChange >= ms && _time - _lastLongPress >= continuous_time) {		HACKHACK VK
	// 	_lastLongPress = _time;
	long t = millis();
	if (_state && t - _lastChange >= ms && t - _lastLongPress >= continuous_time) {
		_lastLongPress = t;
		return true;
	}
	return false;
