...
void WallClock::draw_maximum() {
  ...
  M5.Lcd.fillRect(0, 0, X_WIDTH, Y_HEIGHT, colors[cur_color].bg_color);
  draw_clockface();
  draw_hands(false);
}
...
void WallClock::draw_hands(bool on_second) {
  ...
  RTC_TimeTypeDef td;
  M5.Rtc.GetTime(&td);
  if(on_second && last_second == td.Seconds) return;
  uint8_t hour              = td.Hours;
  uint8_t minute            = td.Minutes;
  uint8_t second            = td.Seconds;
