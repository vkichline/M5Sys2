...
#define SETTINGS_FILE_NAME  "/dat/StoredSettings.json"
...
//
void setup() { sys.begin("SettingsStorage"); }
...
void loop() {
  ...
  menu.addItem("Remove all NVS Settings from M5Stack", clear_settings);
  menu.run();
  sys.load_home_program();
}
