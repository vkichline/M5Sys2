...
// The ISAM database index is expected to be on SD, named /dat/hexagrams.idx
...
bool IChing::begin() {
  ...
  File idx = SD.open("/dat/hexagrams.idx", "r");
  if(!idx) {
...
// The ISAM database file is expected to be on SD, named /dat/hexagrams.isam
...
HexagramInfo* IChing::load_hexagram_from_offset(int offset) {
  ...
  File isam = SD.open("/dat/hexagrams.isam", "r");
  if(!isam) {
