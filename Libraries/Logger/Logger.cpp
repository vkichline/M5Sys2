...
long            Logger::millis_offset;
bool            Logger::include_time;
SysLogLevel_t Logger::log_level;


void Logger::begin(const char* appName, long millis_offset) {
  Logger::program_name  = appName;
  Logger::log_device    = &Serial;
  Logger::log_level     = LOG_NONE;
  Logger::millis_offset = millis_offset;
  if(0 != millis_offset) {
    include_time = true;
  }
  set_from_prefs();
...
SysLogLevel_t Logger::get_level() {
...
void Logger::menu() {
  ...
  Preferences prefs;
   ...
   while(true) {
    bool stat = prefs.begin(M5SYS2_PREFS_NAME, true); // Open in R/W mode
    if(!stat) { error("Logger::menu: failed to open %s\n", M5SYS2_PREFS_NAME); }
    SysLogLevel_t       l_level   = (SysLogLevel_t)prefs.getInt(     "log_level",  LOG_INFO);
    SysLogOutputKind_t  l_kind    = (SysLogOutputKind_t)prefs.getInt("log_kind",   LOG_OUTPUT_SERIAL);
...
void Logger::set(const SysLogLevel_t level) {
...
void Logger::set(const SysLogLevel_t level, Print &device) {
...
void Logger::menu_logging_level() {
  ...
  prefs.begin(M5SYS2_PREFS_NAME, true);
  SysLogLevel_t l_level = (SysLogLevel_t)prefs.getInt("log_level", LOG_INFO);
  prefs.end();
  ...
  SysLogLevel_t new_l_level = l_level;
  ...
  if(new_l_level != l_level) {
    prefs.begin(M5SYS2_PREFS_NAME, false);
...
void Logger::menu_logging_target() {
  ...
  SysLogOutputKind_t  new_l_kind    = l_kind;
...
void Logger::set_from_prefs() {
  ...
  Preferences           prefs;
  prefs.begin(M5SYS2_PREFS_NAME, true);	// read-only
  SysLogLevel_t       l_level   = (SysLogLevel_t)prefs.getInt(     "log_level",  LOG_INFO);
  SysLogOutputKind_t  l_kind    = (SysLogOutputKind_t)prefs.getInt("log_kind",   LOG_OUTPUT_SERIAL);
  bool                  l_append  = prefs.getBool(                     "log_append", false);
  prefs.end();
  ...
  switch(l_kind) {
    ...
    case LOG_OUTPUT_SD_FILE:
      ...
      String name = String("/log/") + program_name + ".log";
      Logger::log_file = SD.open(name, mode);
      output           = &log_file;
      break;
  }
  set(l_level, *output);
...
void Logger::render_time(char* buffer) {
  if(include_time) {
    long  curr_time = (millis() - millis_offset) / 1000;
    uint8_t hours = curr_time / 3600;
    curr_time -= hours * 3600;
    uint8_t minutes = curr_time / 60;
    curr_time -= minutes * 60;
    uint8_t seconds = curr_time;
