...
int checkPsram() {
  ...
  if( testbuff == nullptr ) {
    ...
    CoverScrollText("PSRAM malloc Successful",M5.Lcd.color565(SUCCESS_COLOR));
  }
  delay(100);
  ...
  CoverScrollText("PSRAM W&R Successful",M5.Lcd.color565(SUCCESS_COLOR));
  return 0;
}
...
int checkI2cAddr() {
  ...
  do {
    ...
    Wire1.beginTransmission( lastptr->addr ); // HACKHACK felmue fix
    if (Wire1.endTransmission() == 0 ) {      // HACKHACK felmue fix
...
int checkIMUInit() {
  if( M5.IMU.Init() == 0 ) {
    CoverScrollText("IMU Check Successful",M5.Lcd.color565(SUCCESS_COLOR));
  }
  else {
    CoverScrollText("IMU Check failed",M5.Lcd.color565(FAILED_COLOR));
    sysErrorSkip();
  }
  return 0;
}
...
int checkSDCard() {
  ...
  if( Type == CARD_UNKNOWN || Type == CARD_NONE ) {
    CoverScrollText("SDCard Find failed",M5.Lcd.color565(FAILED_COLOR));
    sysErrorSkip();
  }
  else {
    CoverScrollText("SDCard Found",M5.Lcd.color565(SUCCESS_COLOR));
...
void powerSetup() {
  ...
  DisPowerbuff.drawColorBitmap(0, 39, 77, 24, (uint8_t*)image_power, 0xff9c00, 0x000000);
...
void choosePower() {
  ...
  DisPowerbuff.drawColorBitmap( 0,    0, 77,37, (uint8_t*)image_Internal, color1,   0x000000);
  DisPowerbuff.drawColorBitmap( 0,   65, 77,37, (uint8_t*)image_External, color2,   0x000000);
  DisPowerbuff.drawColorBitmap(54, posy, 20,31, (uint8_t*)image_PowerC,   0xae2828, 0x000000);
  DisPowerbuff.drawColorBitmap( 0,   39, 77,24, (uint8_t*)image_power,    0xff9c00, 0x000000);
  DisPowerbuff.pushSprite(240,3);
...
void clockFlush() {
  ...
  DisClockbuff.drawColorBitmap(posx[0], 26, 24, 42, DigNumber     [sysState.Rtctime.Hours   / 10 ], 0xff9c00, 0x1a1100);
  DisClockbuff.drawColorBitmap(posx[1], 26, 24, 42, DigNumber     [sysState.Rtctime.Hours   % 10 ], 0xff9c00, 0x1a1100);
  DisClockbuff.drawColorBitmap(posx[2], 26, 24, 42, DigNumber     [sysState.Rtctime.Minutes / 10 ], 0xff9c00, 0x1a1100);
  DisClockbuff.drawColorBitmap(posx[3], 26, 24, 42, DigNumber     [sysState.Rtctime.Minutes % 10 ],  0xff9c00, 0x1a1100);
  DisClockbuff.drawColorBitmap(posx[4], 33, 18, 35, DigNumber_35px[sysState.Rtctime.Seconds / 10 ], 0xff9c00, 0x1a1100);
  DisClockbuff.drawColorBitmap(posx[5], 33, 18, 35, DigNumber_35px[sysState.Rtctime.Seconds % 10 ], 0xff9c00, 0x1a1100);
  sysState.clockCount ++;
  sysState.clockCount %= 6;
  DisClockbuff.drawColorBitmap(0, 0, 164, 17, rectptrBuff[sysState.clockCount], 0xff9c00, 0x000000);
...
void batpowerFlush() {
  ...
  DisBatbuff.drawColorBitmap( 3, 20, 7, 10, Number_7x10px[batVoltage1], 0xff9c00, 0x000000);
  DisBatbuff.drawColorBitmap(13, 20, 7, 10, Number_7x10px[batVoltage2], 0xff9c00, 0x000000);
  DisBatbuff.drawColorBitmap(20, 20, 7, 10, Number_7x10px[10],          0xff9c00,  0x000000);
  ...
  while (true) {
    ...
    if ((menuState == 1) || (menuState == 3) || (flushJPEG == true)) {
      if (flushJPEG == true)
        flushJPEG = false;
      if (menuState == 3)
        Disbuff.pushImage(0, 206, 320, 34, buff);
...
void AppImage() {
  ...
  drawJPEGImageFromSDCard(SD, "/img");
}
...
void AppWifi() {
  ...
  WiFi.mode(WIFI_STA);
  WiFi.disconnect();
  Disbuff.drawJpg(wifiScanImage, 28123, 0, 0, 320, 240);
  Disbuff.pushSprite(0, 0);
  ...
  TouchPoint_t pos      = M5.Touch.getPressPoint();
  uint16_t color_yellow = Disbuff.color565(0xff, 0xa0, 0x0);
    ...
    if(WifiNumber == - 1) {
      if(first) {
        if(count >= 10) {
          ...
          Disbuff.drawColorBitmap(122, 98, 76, 44, TouchFishBuff[count_n], 0xff9c00, 0x000000);
          Disbuff.pushSprite(0,0);
          count_n++;
...
void setup()
{
    ...
    sysState.App1Zone[0] = new HotZone( 20,191, 64,235,&AppImage);
    sysState.App1Zone[1] = new HotZone( 79,191,123,235,&AppWifi);
    sysState.App1Zone[2] = new HotZone(138,191,182,235,&AppTimer);
    sysState.App1Zone[3] = new HotZone(197,191,241,235,&APPSleep);
    sysState.App1Zone[4] = new HotZone(256,191,300,235,&AppSetting);
    sysState.App1Zone[5] = new HotZone(72,130,161,186,&MPU6886Page);
    ...
    M5.Axp.SetLDOEnable(3,true);
    CoverScrollText("Motor Test",M5.Lcd.color565(SUCCESS_COLOR));
    delay(100);
    M5.Axp.SetLDOEnable(3,false);

    M5.Axp.SetLed(1);
    CoverScrollText("LED Test",M5.Lcd.color565(SUCCESS_COLOR));
    delay(100);
    M5.Axp.SetLed(0);
